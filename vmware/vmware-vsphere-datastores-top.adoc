...
Las siguientes tablas presentan funciones de almacén de datos tradicionales compatibles con vSphere con ONTAP. Esta información no se aplica a almacenes de datos vVols, pero, generalmente, se aplica a vSphere 6.x y versiones posteriores mediante versiones ONTAP compatibles. También se pueden consultar link:https://www.vmware.com/support/pubs/["Máximos de configuración de VMware"^]versiones específicas de vSphere para confirmar límites específicos.
